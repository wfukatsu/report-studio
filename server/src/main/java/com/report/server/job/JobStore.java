 ...
 *
 * <p>One contract for job metadata persistence and artifact placement: metadata lives in ScalarDB
 * ({@link JobRepository}), artifacts on the local filesystem under {@code data/jobs/{jobId}/}. The
 * V1 batch stack and both V2 stacks (single-PDF and batch) all run on this interface, which gives
 * V2 jobs the same restart resilience ({@code reconcileOrphans}) and TTL reaping as V1.
 */
public interface JobStore {
    ...
    /**
     * Delete jobs whose {@code expiresAt} is set and in the past (records + artifacts). Returns the
