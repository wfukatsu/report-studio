...
/**
 * Immutable job metadata record — the single record type for every job stack
 * (issue #60). Stored as JSON in ScalarDB table {@code report_studio.jobs}.
 *
 * <p>Fields added by the unification are tolerated as absent when reading
 * records written before it: {@code jobType} (null = V1 batch), {@code owner}
 * (null = no ownership restriction), {@code artifactPath}, and
 * {@code expiresAt} (0 = no TTL).
 */
...
public record JobRecord(
    ...
    public JobRecord(String jobId, String templateId, String status,
                     int totalItems, int processedItems, int failedItems,
                     String errorMessage, long createdAt, long updatedAt, long completedAt) {
        this(jobId, templateId, status, totalItems, processedItems, failedItems,
                errorMessage, createdAt, updatedAt, completedAt,
                TYPE_V1_BATCH, null, null, 0);
    }

    /** Fresh PENDING record. */
    public static JobRecord create(String jobId, String templateId, String jobType,
                                   String owner, int totalItems, long expiresAt) {
        long now = System.currentTimeMillis();
        return new JobRecord(jobId, templateId, PENDING, totalItems, 0, 0, null,
                now, now, 0, jobType, owner, null, expiresAt);
    }
    ...
    public JobRecord withStatus(String newStatus) {
        ...
        return new JobRecord(jobId, templateId, newStatus, totalItems,
            ...
            jobType, owner, artifactPath, expiresAt);
    }
    ...
    public JobRecord withArtifact(String path) {
        return new JobRecord(jobId, templateId, COMPLETED, totalItems,
            processedItems, failedItems, errorMessage,
            createdAt, System.currentTimeMillis(), System.currentTimeMillis(),
            jobType, owner, path, expiresAt);
    }
