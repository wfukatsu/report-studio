...
/**
 * Seeds the ScalarDB database with sample tables and data for the built-in
 * business document templates (御見積書, 御注文書, 御請求書).
 *
 * <p>Usage: {@code ./gradlew seed}
 *
 * <p>Tables are created in the {@code sample} namespace. If a table already
 * exists, creation is skipped (idempotent). Rows are upserted so re-running
 * the seed is safe.
 */
public final class SeedData {
    ...
    private static void ensureNamespace(DistributedTransactionAdmin admin) throws ExecutionException {
    ...
    private static void createTables(DistributedTransactionAdmin admin) throws ExecutionException {
        createTableIfAbsent(admin, "customers", orderedMap(
            "id", DataType.TEXT,
        ...
        createTableIfAbsent(admin, "documents", orderedMap(
            "id", DataType.TEXT,
        ...
        createTableIfAbsent(admin, "items", orderedMap(
        ...
        createTableIfAbsent(admin, "summaries", orderedMap(
            "document_id", DataType.TEXT,
        ...
        createTableIfAbsent(admin, "bank_accounts", orderedMap(
            "document_id", DataType.TEXT,
        ...
        createTableIfAbsent(admin, "deliveries", orderedMap(
            "document_id", DataType.TEXT,
    ...
    private static void insertSampleData(DistributedTransactionManager txMgr)
        ...
        try {
            ...
            tx.put(Put.newBuilder()
                .namespace(NS).table("bank_accounts")
