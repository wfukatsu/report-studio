...
/**
 * Document auto-numbering (自動採番) service — the HTTP-independent half of the sequence feature (#419,
 * extracted from {@code SequenceController}).
 *
 * <p>Sequence increment is done atomically within a single ScalarDB DistributedTransaction
 * (read-then-write) with OCC retry. The {@code documentNumber} is stamped into form responses at
 * submission time ({@link #nextAndStamp}) or inside a caller-owned transaction on the first
 * draft→issued status transition ({@link #nextNumberWithinTx}).
 *
 * <p>{@code SequenceController} keeps the config CRUD endpoints; this service is shared by the
 * submit flow ({@code FormResponseController} / {@code ResponseStatusUpdater}) via constructor
 * injection.
 */
public final class DocumentNumberService {
    ...
    public DocumentNumberService(JsonBlobRepository seqRepo) {
     ...
     * ResponseStatusUpdater}) atomically number a document in the same transaction that mutates the
     * document, closing the read-modify-write race (#205).
     */
