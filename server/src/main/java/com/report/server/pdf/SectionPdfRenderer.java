...
/**
 * Interface for section-type-specific PDF rendering.
 * Each implementation handles one section type (page_base, detail_table, etc.).
 *
 * <p>Mirrors the ElementPdfRendererRegistry pattern — add a new section type
 * by creating a new implementation and registering it in SectionPdfRendererRegistry.
 */
public interface SectionPdfRenderer {
    ...
    /**
     * Whether this section type drives pagination (determines total page count).
     * At most one paginating section per template is used for page count calculation.
     */
     ...
     */
    void renderPage(PageContext ctx, JsonNode section, JsonNode formData,
