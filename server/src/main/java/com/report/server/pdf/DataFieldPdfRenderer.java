 ...
 *
 * <p>The bound value arrives via {@code props.text} (resolved upstream from
 * the element's {@code fieldKey} by SectionRenderHelper), then the element's
 * {@code format} ({@code CalculationFormat}) is applied through
 * {@link ValueFormatter} — the server port of src/lib/numberFormatter.ts.
 * Empty values fall back to {@code fallbackText} (blank when absent; the
 * designer-only gray label placeholder is intentionally not reproduced).
 *
 * <p>Style support ({@code TextStyle} subset): fontSize (pt), bold,
 * fontFamily (serif/mincho switch), color, textAlign (left/center/right).
 */
public final class DataFieldPdfRenderer implements ElementPdfRenderer {
    ...
    public void render(PDPageContentStream cs, JsonNode el, float x, float y,
        ...
        if (raw.isEmpty()) {
            ...
            text = ValueFormatter.applyFormat(
                    props != null && props.has("text") ? props.get("text") : el.path("text"),
                    format);
        }
