 ...
 *
 * <p>Supports multi-line wrapping, horizontal alignment, the vertical writing
 * mode (縦書き, {@code writingMode: "vertical-rl"}), furigana (ruby), and
 * synthetic bold (stroke-widened glyphs, since only a Regular CJK face is
 * bundled). Reads the string as props.text → content → name and the style
 * from props or the V2 {@code style} object.
 */
public final class TextPdfRenderer implements ElementPdfRenderer {
    ...
    public void render(PDPageContentStream cs, JsonNode el, float x, float y,
        ...
        float lineHeight = firstNonZero(style != null ? floatOf(style, "lineHeight", 0) : 0,
                DEFAULT_LINE_HEIGHT);
        boolean vertical = "vertical-rl".equals(firstNonEmpty(
        ...
        try {
            ...
            if (vertical) {
                ...
                drawHorizontal(cs, font, fontSize, lineHeight, text, x, y, w, align, furigana, furiScale);
            }
        } finally {
    ...
    private static void drawHorizontal(PDPageContentStream cs, PDFont font, float fontSize,
                                       float lineHeight, String text, float x, float y, float w,
                                       String align, String furigana, float furiScale) throws IOException {
    ...
    private static void drawVertical(PDPageContentStream cs, PDFont font, float fontSize,
                                     float lineHeight, String text, float x, float y, float w, float h,
                                     String furigana, float furiScale) throws IOException {
        ...
        for (int i = 0; i < cps.length; i++) {
            int cp = cps[i];
            if (cp == '\n') { colX -= colStep; row = 0; continue; }
            if (row >= maxPerCol) { colX -= colStep; row = 0; }
    ...
    private static void configureBold(PDPageContentStream cs, boolean bold, float fontSize, Color color)
            throws IOException {
        if (bold) {
    ...
    private static void showLine(PDPageContentStream cs, PDFont font, float size,
                                 String text, float tx, float baselineY) throws IOException {
        if (text.isEmpty()) return;
    ...
    private static String sanitizeForFont(PDFont font, String text) {
        try {
            ...
            text.codePoints().forEach(cp -> {
                String ch = new String(Character.toChars(cp));
                try { font.getStringWidth(ch); sb.append(ch); }
    ...
    private static float strWidth(PDFont font, String s, float size) {
        try { return font.getStringWidth(sanitizeForFont(font, s)) / 1000 * size; }
        catch (Exception e) { return s.length() * size; }
    }

    private static float firstNonZero(float a, float b) { return a != 0 ? a : b; }
    private static float firstNonZero(float a, float b, float c) {
        return a != 0 ? a : (b != 0 ? b : c);
    }
    private static String firstNonEmpty(String a, String b) { return !a.isEmpty() ? a : b; }
}
