 ...
 *
 * <p>Table schema: template_id (PK) + version_id (CK) + json_data + label + created_at + is_auto
...
public final class VersionRepository {
    ...
    public record VersionMeta(
            String versionId, String templateId, String label, long createdAt, boolean isAuto) {}
    ...
    public String createVersion(String templateId, String json, String label, boolean isAuto) {
        ...
        try {
            ...
            throw new JsonBlobRepository.RepositoryException(
                    "Failed to create version for " + templateId, e);
        }
    }
    ...
    public List<VersionMeta> listVersions(String templateId) {
        ...
        try {
            ...
            abortQuietly(tx);
            throw new JsonBlobRepository.RepositoryException(
                    "Failed to list versions for " + templateId, e);
        }
    }
    ...
    public Optional<String> getVersion(String versionId) {
        ...
        try {
            ...
            abortQuietly(tx);
            throw new JsonBlobRepository.RepositoryException(
                    "Failed to get version " + versionId, e);
        }
    }
    ...
    public void updateLabel(String versionId, String label) {
        ...
        try {
            ...
            abortQuietly(tx);
            throw new JsonBlobRepository.RepositoryException(
                    "Failed to update label for " + versionId, e);
        }
    }
    ...
    /**
     * Create an auto-snapshot if enough time has passed since the last one. Also prunes old
     * auto-versions exceeding MAX_AUTO_VERSIONS.
     */
    public boolean createAutoVersionIfNeeded(String templateId, String json) {
        ...
        Optional<VersionMeta> lastAuto = versions.stream().filter(VersionMeta::isAuto).findFirst();
        ...
        List<VersionMeta> autoVersions = versions.stream().filter(VersionMeta::isAuto).toList();
