...
/**
 * Fixed-window rate limiter with automatic cleanup of expired entries. Allows a maximum number of
 * attempts per key within a time window.
 *
 * <p>Default: 5 attempts per 5 minutes per key (login / form-password use cases). Pass custom
 * values to the constructor for other use cases.
 */
public final class RateLimiter {
    ...
    /**
     * Check if a request for the given key is allowed. Increments the counter if allowed.
     *
    ...
    public boolean isAllowed(String key) {
        ...
        if (!allowed) {
            // Observability: count rejected requests across all limiters (login, export, row
            // writes, …).
     ...
     *
     * <p>Used to forgive a key's accumulated attempts after a legitimate outcome — e.g. a
     * successful login — so that brute-force protection targets failed attempts rather than
     * throttling genuine users (notably several users behind a shared/NAT IP).
     */
