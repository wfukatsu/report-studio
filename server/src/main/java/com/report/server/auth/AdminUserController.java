 ...
 *
 * <ul>
 *   <li>GET /api/v1/admin/users — list all users (passwords masked)
 *   <li>POST /api/v1/admin/users — create user
 *   <li>PUT /api/v1/admin/users/{id} — update displayName, password, roles
 *   <li>DELETE /api/v1/admin/users/{id} — delete user (cannot delete self)
 * </ul>
 *
 * All endpoints require the "admin" role. The role check is NOT performed here — it is enforced by
 * the /api/v1/admin/* before-filter registered in ApiRoutes.registerAdminRoleFilter (pinned by
 * AdminRoleFilterWiringTest).
 */
public final class AdminUserController {
    ...
    public void create(Context ctx) {
        ...
        List<String> rolesList =
                body.get("roles") instanceof List<?> l ? (List<String>) l : List.of("user");
    ...
    public void update(Context ctx) {
        ...
        Set<String> updatedRoles =
                body.get("roles") instanceof List<?> l
                        ? Set.copyOf((List<String>) l)
                        : user.roles();
