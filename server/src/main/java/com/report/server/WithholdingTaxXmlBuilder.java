 ...
 * <ul>
 *   <li>DOCTYPE declarations are disallowed (XXE prevention)</li>
 *   <li>External entity resolution is disabled</li>
 *   <li>All field values are set via DOM {@code setTextContent()} — never string concatenation</li>
 * </ul>
 *
...
public final class WithholdingTaxXmlBuilder {
     ...
     *
     * <p>Use {@code "stub-1.0"} for the bundled stub. When the official NTA XSD is
     * registered in {@link EtaxXmlValidator}, pass the corresponding version key.
     *
     * @throws XmlValidationException if the generated XML does not conform to the schema
     */
    public static Document buildAndValidate(JsonNode projection, JsonNode formData,
    ...
    /**
     * Append a child element with the given text content.
     * Uses DOM {@code setTextContent()} so the DOM serializer handles all escaping —
     * {@code <}, {@code >}, {@code &} are never injectable through this path.
     */
    ...
    /**
     * Create a DocumentBuilderFactory with all external entity / DOCTYPE features disabled.
