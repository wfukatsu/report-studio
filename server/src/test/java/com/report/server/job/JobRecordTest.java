...
/**
 * Serialization compatibility of the unified JobRecord (issue #60): rows written before the
 * unification lack the new fields and must still parse.
 */
class JobRecordTest {
    ...
    void roundTrip_preservesUnifiedFields() throws Exception {
        JobRecord record =
                JobRecord.create("pjob-1", "t1", JobRecord.TYPE_V2_PDF, "alice", 1, 9999L);
    ...
    void cancelledStatus_isTerminal() {
        JobRecord record =
                JobRecord.create("job-1", "t1", JobRecord.TYPE_V1_BATCH, null, 1, 0)
    ...
    void withArtifact_marksCompleted() {
        JobRecord record =
                JobRecord.create("pjob-1", "t1", JobRecord.TYPE_V2_PDF, null, 1, 0)
    ...
    void compatConstructor_defaultsToV1Batch() {
        JobRecord record =
                new JobRecord("job-1", "t1", JobRecord.PENDING, 1, 0, 0, null, 1L, 1L, 0L);
        assertTrue(record.isV1());
