...
/**
 * Unit tests for {@link SystemValueResolver} (issue #54) — mirrors the
 * frontend formatters in src/elements/pageNumber/format.ts and
 * src/elements/currentDate/format.ts.
 */
class SystemValueResolverTest {
    ...
    void pageNumber_formatStringIsTheTemplate() {
        ...
        assertEquals("3 / 7", SystemValueResolver.formatPageNumber("{{page}} / {{pages}}", null, 3, 7));
        assertEquals("- 3 -", SystemValueResolver.formatPageNumber("- {{page}} -", null, 3, 7));
        assertEquals("3ページ (全7ページ)",
                SystemValueResolver.formatPageNumber("{{page}}ページ (全{{pages}}ページ)", null, 3, 7));
    }

    @Test
    void pageNumber_customUsesCustomFormat() {
        assertEquals("P3/7", SystemValueResolver.formatPageNumber("custom", "P{{page}}/{{pages}}", 3, 7));
    ...
    void currentDate_customPatternWithDayOfWeek() {
        ...
        assertEquals("2026.07.16", SystemValueResolver.formatCurrentDate("custom", "yyyy.MM.dd", d));
        assertEquals("07/16 (木)", SystemValueResolver.formatCurrentDate("custom", "MM/dd (ddd)", d));
    ...
    void currentDate_unknownFormatFallsBackToSlashed() {
        assertEquals("2026/07/16",
                SystemValueResolver.formatCurrentDate("bogus", null, LocalDate.of(2026, 7, 16)));
    }

    @Test
    void currentDate_reiwaGannen() {
        assertEquals("令和1年05月01日",
                SystemValueResolver.formatCurrentDate("wareki_full", null, LocalDate.of(2019, 5, 1)));
    }
}
