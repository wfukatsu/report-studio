...
/**
 * Integration tests for the Report Design Studio API.
 * Uses Javalin testtools to spin up a real HTTP server per test.
 * ScalarDB-dependent endpoints (projections, schemas) are excluded
 * since they require a full TransactionFactory setup.
 */
class ApiIntegrationTest {
    ...
    /**
     * Build a minimal Javalin app with only the non-ScalarDB routes.
     * Mirrors App.java route registration for health, auth, and template CRUD.
     */
    private Javalin createTestApp() {
        ...
        app.get("/api/v1/templates/{id}/designer-projection", ctx -> {
            String id = RequestValidator.validateId(ctx);
            if (id == null) return;
    ...
    class TemplateCrud {
        ...
        void createReturns201WithIdAndName() {
            JavalinTest.test(createTestApp(), (server, client) -> {
                try (Response response = client.post("/api/v1/templates",
                        Map.of("name", "テスト帳票"))) {
        ...
        void listContainsCreatedTemplate() {
            JavalinTest.test(createTestApp(), (server, client) -> {
                // Create a template
                try (Response createResp = client.post("/api/v1/templates",
                        Map.of("name", "一覧テスト"))) {
        ...
        void renameUpdatesName() {
            JavalinTest.test(createTestApp(), (server, client) -> {
                ...
                try (Response patchResp = client.request("/api/v1/templates/" + id,
    ...
    class TemplateNameValidation {
        ...
        void rejectsNameTooLong() {
            JavalinTest.test(createTestApp(), (server, client) -> {
                ...
                try (Response resp = client.post("/api/v1/templates",
