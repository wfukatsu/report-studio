...
class BatchFilenameTest {
    ...
    void nullTemplate_fallsBackToSeqAndDate() {
        assertEquals("003_20260720.pdf",
                BatchPdfController.buildFilename(null, "003", "20260720", "", "", data()));
    }

    @Test
    void expandsBuiltInTokens() {
        assertEquals("INV-0007_issued.pdf",
                BatchPdfController.buildFilename("{documentNo}_{status}.pdf", "003", "20260720",
                        "INV-0007", "issued", data()));
    }

    @Test
    void expandsFlatAndNestedDataFields() {
        assertEquals("評価商事_山田太郎.pdf",
                BatchPdfController.buildFilename("{name}_{customer.name}.pdf", "001", "20260720",
                        "", "", data()));
    }
    ...
    void unknownTokensResolveEmptyAndCollapse() {
        ...
        assertEquals("A.pdf",
                BatchPdfController.buildFilename("{missing}_A_{missing}.pdf", "001", "20260720",
                        "", "", data()));
    }

    @Test
    void appendsPdfExtensionWhenMissing() {
        assertTrue(BatchPdfController.buildFilename("{seq}", "005", "20260720", "", "", data())
                .endsWith(".pdf"));
    }

    @Test
    void sanitizesPathSeparatorsAndUnsafeChars() {
        String out = BatchPdfController.buildFilename("{name}", "001", "20260720",
                "", "", jsonWith("name", "a/b:c*d"));
