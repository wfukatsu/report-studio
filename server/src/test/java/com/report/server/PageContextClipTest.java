...
/**
 * Tests for opt-in margin clipping (issue #55 — {@code pageSetup.clipToMargins}). Clipping does not
 * remove text from the content stream, so the assertions inspect the stream for the clip operator
 * ({@code W}) instead of parsing text.
 */
    ...
    private static boolean hasClipOperator(byte[] pdf) throws Exception {
        try (PDDocument doc = Loader.loadPDF(pdf)) {
            ...
            return tokens.stream()
                    .anyMatch(t -> t instanceof Operator op && "W".equals(op.getName()));
        }
    }
