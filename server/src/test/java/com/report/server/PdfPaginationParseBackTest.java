 ...
 *
 * <p>Covers height-derived row capacity, page-count math, header repetition, row rendering across
 * pages (row_block), independent pagination of multiple repeating sections, and unit-based
 * multi_row_table flow.
 */
class PdfPaginationParseBackTest {

    /**
     * detail_table with 12 rows. Section y=20, rows start at y=32 with 8mm stride; {@code
     * sectionHeight} controls the height-derived capacity: height 92 → bottom edge 112 → (112−32)/8
     * = 10 rows/page.
     */
    private static String detailTableJson(String extraSectionFields, int sectionHeight) {
        ...
        return """
            ...
            "_formData":{"items":[%s]}}"""
                .formatted(sectionHeight, extraSectionFields, items);
    }
    ...
    void heightDerivedCapacity_tenRowsPerPage_yieldsTwoPages() throws IOException {
        PdfProbe probe =
                PdfProbe.parse(PdfRenderer.render(detailTableJson("\"repeatHeader\":true,", 92)));
    ...
    void heightDerivedCapacity_fiveRowsPerPage_yieldsThreePages() throws IOException {
        ...
        PdfProbe probe =
                PdfProbe.parse(PdfRenderer.render(detailTableJson("\"repeatHeader\":true,", 52)));
    ...
    void fixedRowMode_fixedRowCountOverridesGeometry() throws IOException {
        PdfProbe probe =
                PdfProbe.parse(
                        PdfRenderer.render(
                                detailTableJson(
                                        "\"tableMode\":\"fixed\",\"fixedRowCount\":5,", 92)));
    ...
    void twoRepeatingSections_paginateIndependently() throws IOException {
        ...
        String json =
            ...
            "_formData":{"itemsA":[%s],"itemsB":[%s]}}"""
                        .formatted(itemsA, itemsB);
    ...
    void multiRowTable_unitsAdvanceByUnitExtentAndNeverSplit() throws IOException {
        ...
        PdfProbe.TextRun name2 =
                probe.findRun(0, "記録二").orElseThrow(() -> new AssertionError(probe.dumpRuns()));
