...
/**
 * Parse-back tests for tenant* elements (issue #54). The tenant document is
 * supplied via the projection's {@code _tenant} override (production wiring
 * goes through TenantInfoProvider, registered in AppWiring).
 */
class PdfTenantParseBackTest {
    ...
    private static final String TENANT = """
        {"companyName":"株式会社スカラー",
         ...
         "logoBase64":"%s",
         "custom":{"registrationNumber":"T1234567890123"}}""".formatted(LOGO_DATA_URI);
        ...
        return """
            {"templates":[{
              ...
              "sections":[{
                ...
                tenantJson == null ? "" : ",\"_tenant\":" + tenantJson);
    }
    ...
    void tenantAddress_singleAndMultiline() throws IOException {
        ...
        assertTrue(probe.pageContains(0, "〒163-0207 東京都新宿区西新宿2-6-1新宿住友ビル7F"),
                probe.pageText(0));

        // multiLine: three separate baselines, 1.2 line-height apart
        // (filter y > 35mm to skip the single-mode element's concatenated run)
        PdfProbe.TextRun postal = probe.findRuns("〒163-0207").stream()
                .filter(r -> r.baselineYMm() > 35).findFirst().orElseThrow();
        PdfProbe.TextRun addr2 = probe.findRuns("新宿住友ビル7F").stream()
                .filter(r -> r.baselineYMm() > 35).findFirst().orElseThrow();
        assertTrue(addr2.baselineYMm() > postal.baselineYMm() + 5,
