...
1. （任意）`npm run seed:samples` でサンプル帳票 6 種（見積書・請求書のほか、継続ページ分割を実演する売上明細一覧など）を公開テンプレートとして投入（要バックエンド起動）
...
4. **エクスポート** から PDF を出力 — サーバーサイド PDF はページ分割・和文フォント埋め込みに対応し、キャンバス側も同一の Noto フォント（自己ホスト Web フォント）で描画されるためプレビューと出力の見た目が一致します
...
| `LOCAL_LOGIN_ENABLED` | `true` | `false` で ID/パスワードログインを無効化（OIDC 設定時のみ有効） |
