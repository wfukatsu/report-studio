...
- All master groups of a template share one `_header` row (PK `doc_no`). Only the **primary** master group (the one whose field maps to `doc_no`) exposes a key input; every other bound group — aux masters (`customer`/`summary`/…) and the detail (`items`) group — links to the primary via `linkedMasterGroupId` and has its `doc_no` auto-filled at preview time (so the designer enters one value, and no synthetic "書類番号（キー）" field clutters the aux groups). The detail group binds to `_items` (PK `doc_no`, CK `line_no`). Date fields are stored as `TEXT` (their pre-formatted display string) so live rows render identically to the sample data.
