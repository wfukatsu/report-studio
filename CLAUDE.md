...
- **Surrogate key**: each `_header` row is keyed by a surrogate `report_id` (UUID PK), with the human-facing `doc_no` kept as a unique business column — identity is not coupled to the document number. All master groups of a template share the one `_header` row; only the **primary** master group exposes a key input (a `reportId`→`report_id` field), and every other bound group — aux masters (`customer`/`summary`/…) and the detail (`items`) group — links to the primary via `linkedMasterGroupId` and has its `report_id` auto-filled at preview time. The detail group binds to `_items` (PK `report_id`, CK `line_no`).
- **Product master**: coded line items reference the shared product master (`report_studio$products`, seeded by `seed:demo-products`) by `product_code` (the `itemCode` field maps to a `product_code` FK column), while name/price are **snapshotted** into the items row for historical accuracy. The three "modern" templates use coded items; the quotation-{basic,discount,english} templates use free-text lines (no product_code). Date fields are stored as `TEXT` (pre-formatted display string) so live rows render identically to the sample data.
- **Regenerate bindings**: `npm run setup:template-bindings` — rewrites the six `src/templates/builtin/*.json` (adds/flattens schema, sets `tableMeta`/`dbColumnName`, the `reportId` key field) and emits `scripts/demo-db-seed.json`. Idempotent.
- **Seed everything**: with the backend up, `npm run seed:demo` runs three steps (also available separately):
  - `seed:demo-products` — seeds the product master (`POST /api/v1/products`, idempotent via code-uniqueness sentinel).
  - `seed:demo-tables` — logs in as `admin`, creates the 12 tables via `POST /api/v2/scalardb/tables` and upserts rows (DELETE-then-INSERT, re-runnable). Raw SQLite writes are wrong — rows must go through ScalarDB's transactional layer. Note: ScalarDB can't ALTER a PK, so changing a table's key requires dropping the `demo$*` SQLite tables + their `scalardb$metadata` rows first.
