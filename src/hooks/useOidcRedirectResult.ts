...
import { useOidcProviderName } from './useOidcProviderName'
...
export function useOidcRedirectResult() {
  ...
  useEffect(() => {
    ...
    if (result.linked) toast.success(t('accountTab.oidcLink.success', { provider }))
    if (result.error) toast.error(t(`loginModal.oidcError.${result.error}`, { provider }))
  }, [authLoading, signedIn, t, provider])
}
