...
elements.push(
  ...
  df('quotation.issueDate',    RIGHT_X + 24, Y_RIGHT_INFO,    RIGHT_W - 24, INFO_ROW, { fontSize: 8 },
    undefined, '20XX年XX月XX日'),
  lbl('見積書No：', RIGHT_X,      Y_RIGHT_INFO + INFO_ROW,     24, INFO_ROW, { fontSize: 8, textAlign: 'right' }),
  df('quotation.number',       RIGHT_X + 24, Y_RIGHT_INFO + INFO_ROW, RIGHT_W - 24, INFO_ROW, { fontSize: 8 },
    undefined, 'M-12345678'),
  lbl('登録番号：', RIGHT_X,      Y_RIGHT_INFO + INFO_ROW * 2, 24, INFO_ROW, { fontSize: 8, textAlign: 'right' }),
  df('quotation.registrationNo', RIGHT_X + 24, Y_RIGHT_INFO + INFO_ROW * 2, RIGHT_W - 24, INFO_ROW, { fontSize: 8 },
...
elements.push(
  ...
  df('quotation.customer.postalCode', ML + 5, Y_CUST_ZIP + 2, LEFT_W - 5, 5, { fontSize: 8 },
    undefined, '000-0000'),
  // 住所
...
// 発行者詳細
elements.push(
  df('quotation.sender.address', RIGHT_X, Y_SENDER_ADDR + 8, RIGHT_W, 6, { fontSize: 8 },
    undefined, '東京都●●区△△△△'),
  lbl('Tel：',  RIGHT_X, Y_SENDER_TEL + 8,  9,  6, { fontSize: 8 }),
  df('quotation.sender.tel',     RIGHT_X + 9,  Y_SENDER_TEL + 8,  RIGHT_W - 9,  6, { fontSize: 8 },
    undefined, '00-0000-0000'),
  lbl('Mail：', RIGHT_X, Y_SENDER_MAIL + 8, 12, 6, { fontSize: 8 }),
  df('quotation.sender.email',   RIGHT_X + 12, Y_SENDER_MAIL + 8, RIGHT_W - 12, 6, { fontSize: 8 },
    undefined, '●●●@example.com'),
  lbl('担当：', RIGHT_X, Y_SENDER_CONT + 8, 12, 6, { fontSize: 8 }),
  df('quotation.sender.contact', RIGHT_X + 12, Y_SENDER_CONT + 8, RIGHT_W - 12, 6, { fontSize: 8 },
    undefined, 'アドビ太郎'),
)
...
summRows.forEach(({ label, key, y, bold }, i) => {
  ...
  elements.push(
    lbl(label, SUMM_LABEL_X + 1, y, SUMM_LABEL_W - 2, SUMM_ROW_H,
      { fontSize: 8, textAlign: bold ? 'center' : 'left', fontWeight: bold ? 'bold' : 'normal' }),
