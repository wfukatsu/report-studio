...
describe('DataBindingOverviewPanel — サンプルデータからのパーティションキー自動入力', () => {
  it('DBバインド済みテンプレート読込時に report_id がサンプル値で自動入力される', async () => {
    mockAnalysis({ hasDataSource: true })
    ...
    store.addSchemaField(gid, { key: 'reportId', label: '帳票ID', type: 'string', dbColumnName: 'report_id' } as import('@/types').SchemaField)
    ...
    await vi.waitFor(() => {
      ...
      expect(input.value).toBe('RPT-0001')
    })
  })
})
...
describe('DataBindingOverviewPanel — 親リンクのマスターグループ（#133）', () => {
  it('linkedMasterGroupId を持つマスターグループは "(自動: ...)" 表示で手動入力を隠す', () => {
    ...
    store.addSchemaField(primaryId, { key: 'reportId', label: '帳票ID', type: 'string', dbColumnName: 'report_id' } as import('@/types').SchemaField)
