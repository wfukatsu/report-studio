        ...
        <div className="mt-1 space-y-1">
          {el.cells.map((cell, i) => (
            <div key={i} className="flex gap-1 items-center">
              <input type="text" className="border rounded px-1.5 py-0.5 text-xs flex-1 bg-background" value={cell.role} onChange={(e) => { const cells = el.cells.map((c, ci) => ci === i ? { ...c, role: e.target.value } : c); onChange({ cells }) }} />
