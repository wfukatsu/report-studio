...
- [x] DbConnectionTab.tsx のエラー表示に `role="alert"` 追加
- [x] CreateTableForm.tsx のエラー表示に `role="alert"` 追加
- [x] Toolbar.tsx の exportError / refreshError 表示に `role="alert"` 追加
- [x] スクリーンリーダー（VoiceOver 等）でエラー発生時に自動読み上げされる
