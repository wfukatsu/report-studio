...
| 自動/テナントフィールド | リテラルトークン/書式（`{{会社名}}`・`yyyy/MM/dd`） | 解決値（テナント値も `fallback` も未設定なら**非表示** — サーバ PDF と一致、#315） |
...
各要素は `src/elements/{type}/` に自己完結（`Renderer.tsx` / `PropertiesPanel.tsx` / `index.tsx`（ElementDef）/ `*.stories.tsx` / `*.test.tsx`）し、共通部品から合成します。
...
`_blocks/` は上記の合成ビルディングブロック（renderers / hooks / panels）。汎用の UI プリミティブ（`sharedUI.tsx` の PropSection / PropRow / NumInput、`styleUtils.ts`、`ColorPickerPopover`）は `_base/` にあり、新要素は `_blocks/` から合成し、汎用入力部品を足すときのみ `_base/` に置きます（#436）。

ディスパッチは**要素レジストリ**に集約（#414）。各要素は `src/elements/{type}/index.tsx` で `ElementDef`（`renderElement` アダプタ・`PropertiesPanel`・`layerIcon` / `layerName`・`bindable`・任意の `isEmptyInPreview`）をエクスポートし、`src/elements/registry.ts` の `ELEMENT_REGISTRY` に登録します。この map は `ReportElement['type']` 直和上の `Record` なので、登録漏れ・余分キーは**コンパイルエラー**（旧来の per-file `assertNever` と同じ網羅保証を 1 箇所に集約）。`components/canvas/ElementRenderer.tsx` は `getElementDef(type).renderElement(ctx)` へ委譲するだけの薄いディスパッチャで、readonly/design の差異は各 def の `renderElement` アダプタが吸収します。ElementRenderer / サイドバー PropertiesPanel / `layerUtils` / `BINDABLE_TYPES` / プレビュー空判定（`isDataEmptyInPreview`）はすべてこのレジストリから引かれます。

## 4. 要素タイプの追加手順

1. `src/types/index.ts` に `ElementBase` を継承する `XxxElement` インターフェースを定義し、`ReportElement` 直和に追加。
2. `src/types/elementTypes.ts` の `ELEMENT_TYPES` に型文字列を追加（直和との網羅性は双方向にコンパイル強制、#415）し、`npm run generate:schema` を実行。
3. `src/elements/xxx/Renderer.tsx` を作成（`_blocks/renderers/` から合成）。
4. `src/elements/xxx/PropertiesPanel.tsx` を作成（`_blocks/panels/` から合成）。
5. `src/elements/xxx/index.tsx` で `ElementDef`（`xxxDef`）をエクスポート（`renderElement` アダプタ・`PropertiesPanel`・`layerIcon` / `layerName`・`bindable`・任意の `isEmptyInPreview`）。
6. `src/elements/registry.ts` の `ELEMENT_REGISTRY` に登録（`Record` なので登録漏れ・余分キーはコンパイルエラー）。
7. `lib/elementFactories.ts` にファクトリを追加。
8. `components/sidebar/paletteData.tsx` にパレット項目を追加（`registry.test.ts` が palette/factory ↔ registry の整合を検証）。
9. サーバー側 PDF 出力が必要なら `com.report.server.pdf` に要素レンダラを追加し `ElementPdfRendererRegistry` に登録。
...
com.report.server                app 起動・ルーティング・エンジン・Controller・Repository
  ...
  ScalarDbGateway                         ScalarDB アクセスの集約点（#421）
  Repositories: JsonBlobRepository, ProjectionRepository, VersionRepository,
                StatusAuditRepository, TemplateListRepository（ファイルベース）
  Services: DocumentNumberService, ProductCatalogService, WebhookDispatchService
  Controllers                             各機能の HTTP ハンドラ（リソース別・V2 プレフィックス無し。
                                          FormResponseController は
