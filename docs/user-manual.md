...
| **発行済み帳票** | 全テンプレート横断の発行済み文書一覧（発行番号・状態・発行日時、ステータス変更・PDF 再取得） |
| **データブラウザ** | ScalarDB テーブル・商品マスター・回答データの閲覧/編集 |
| **ジョブ** | 一括 PDF 等のジョブ履歴（種別・状態・進捗、結果 ZIP のダウンロード・キャンセル/削除） |
| **管理** | ユーザー管理・サーバー設定・テナント情報・デフォルトスタイル・APIトークン・テンプレート（`admin` ロール必須） |
...
| 記入欄 | 記入欄（手書き記入用） |
| 日本語帳票専用 | 印鑑、多段印鑑欄、収入印紙欄、チェックボックス、元号選択（明/大/昭/平/令） |
...
- **要素**: 要素名（レイヤー表示用）、表示/非表示、**ロック**、**印刷**（印刷対象フラグ）、**表示条件** の編集。
...
- **スタイルのコピー/貼り付け**: **スタイルをコピー** / **スタイルを貼り付け** で書式だけを別要素へ複製。スタイルをコピーすると、クリップボードにスタイルが入っている間はボタンの右上に小さなドットが表示されます。
...
- **表示オプション**: グリッドを表示、グリッドにスナップ、トンボを表示、余白ガイドを表示、ヘッダー/フッター編集モード。いずれかのオプションが ON のとき **表示オプション** ボタンの右上に小さなドットが表示されます（青い塗りはメニューを開いている間のみ）。余白ガイドは初期状態で ON なので、最初からドットが付いています。
...
ツールバーのシールドボタン **バリデーション実行**（保存済みテンプレートが必要）で検証を実行します。違反があるとボタンに赤い件数バッジと警告アイコンが表示されます（ボタン自体は青く塗られません）。警告がある状態で出力しようとすると **バリデーション警告** の確認ダイアログが出ます。
...
- 列は **商品コード / 商品名 / カテゴリ / 単価 / 在庫数 / 税区分**。
- 列ヘッダーをクリックすると昇順（↑）・降順（↓）に並べ替わります（もう一度クリックで反転）。
- 検索ボックス（**商品コード・商品名で検索**）は **商品コード** と **商品名** を対象に絞り込みます。
- 商品数が多い場合は **50 件ごとにページ送り**（‹ / › で移動、現在ページ / 総ページ数を表示）されます。

![商品マスター一覧 — 商品コード/商品名/カテゴリ/単価/在庫数/税区分の列](images/manual/09-master-list.png)
...
| 項目 | 説明 |
|------|------|
| **商品コード**（必須） | テナント内で一意。重複すると保存時にエラーになります |
| **商品名**（必須） | 商品名 |
| **単価** | 基準となる単価（数値） |
| **税区分** | なし / 標準税率 / 軽減税率。実際の税率は テナント情報の税率設定 + JEXL `taxRates` で解決されます |
| **カテゴリ / 単位 / 在庫数 / メーカー / 説明** | 任意の付帯情報（単位は「個」「本」「kg」など） |
| **サブスクリプション期間 / 価格単位** | 継続課金商品のときのみ設定 |
| **カスタムフィールド** | 6.6 で定義した項目がここに表示され、値を入力できます |

![商品編集ダイアログ — 商品コード/商品名/単価/税区分/カテゴリ/単位/在庫数/メーカーなどの項目](images/manual/10-master-edit.png)

- **価格履歴**: 既存商品の編集画面では、単価の変更履歴（適用日・価格）が新しい順に表示されます（閲覧のみ）。
- **同時更新の保護**: 他の利用者が先に更新していた場合は「他のユーザーが同じ商品を更新しました。最新データを確認してから再試行してください。」と表示され、上書きを防ぎます。最新を読み直してから再編集してください。
...
- **ユーザーメニュー**（右上）: 表示名・**設定**（**アカウント設定** — 表示名の変更・**パスワード変更**）・**ログアウト**。ログアウト時は現在のテンプレートと自動保存データがクリアされます（ユーザー間の分離）。Keycloak でログインしたセッションからログアウトすると、Keycloak 側の SSO セッションも終了します（RP-Initiated Logout）。
- **管理タブ**（`admin` ロール必須）: **ユーザー管理**（ユーザーの作成/一覧/削除）、**サーバー設定**、**テナント情報**、**デフォルトスタイル**、**APIトークン**、**テンプレート**。

### Keycloak（OIDC）でログインする

サーバーが `AUTH_MODE=both` または `AUTH_MODE=oidc` で起動している場合、Keycloak（OpenID Connect）でログインできます。設定方法は [導入方法 › Keycloak（OIDC）ログインの併用](./setup.md#keycloakoidcログインの併用) を参照してください。

- **ログインモーダル**: ユーザーID/パスワードのフォームの上に **Keycloak でログイン** ボタンが表示されます。`both` ではフォームとの間に「または」の区切りが入り、`oidc` ではフォームは表示されません。ボタン名の「Keycloak」部分は `OIDC_PROVIDER_NAME` に従います（Entra ID / Auth0 などでも利用可）。

  ![ログイン画面 — 「Keycloak でログイン」ボタンと ID/パスワードのフォーム（AUTH_MODE=both）](images/manual/13-login-keycloak.png)

- **初回ログイン**: Keycloak で認証されると、アカウントが自動作成されます（パスワードなし。ロールは IdP のロールから `admin` / `user` にマッピングされ、ログインのたびに更新されます）。
- **エラー**: Keycloak からの戻り（コールバック）に失敗すると、ログインモーダルに理由が表示されます。
  - 「この Keycloak アカウントではログインできません。管理者に連絡してください」（同じユーザーIDのローカルアカウントが既にある等。理由はサーバーログにのみ出ます）
  - 「このアプリを利用するロールが割り当てられていません。管理者に連絡してください」
  - 「Keycloak に接続できません。しばらく待ってから再試行してください」
  - 「Keycloak から受け取ったトークンを検証できませんでした」/「Keycloak でのログインに失敗しました」
  - 「ログインセッションが無効です。もう一度お試しください」/「ログイン試行回数が多すぎます。しばらく待ってから再試行してください」
  - 「このサーバではアカウント連携が無効になっています」/「アカウント連携にはローカルアカウントでのログインが必要です。ログインし直してから再度お試しください」（下記の連携時）
- **アカウント設定**: Keycloak で作成されたアカウントでは、パスワード欄の代わりに「このアカウントのパスワードは Keycloak で管理されています」と表示されます（表示名は変更できます）。
- **ローカルアカウントとの連携**（`OIDC_LINK_LOCAL_USERS=true` の場合）: ローカルアカウント（ID/パスワード）でログインした状態で **アカウント設定** を開くと、**Keycloak 連携** セクションに **Keycloak アカウントと連携** ボタンが表示されます。クリックすると Keycloak のサインイン画面に移り、戻ってくると「Keycloak アカウントと連携しました」と表示されます。以降はパスワードと Keycloak のどちらでもログインでき、アカウント設定には「このアカウントは Keycloak アカウントと連携済みです」と表示されます。

  ![アカウント設定 — ローカルユーザーに表示される「Keycloak アカウントと連携」ボタン](images/manual/14-account-link.png)

- **管理 → ユーザー管理**: 一覧に **認証** 列が追加され、各ユーザーが **ローカル** / **Keycloak** のどちらでログインするかを示します。Keycloak のユーザーにホバーすると「パスワードなし（Keycloak でログイン）」のヒントが出ます。Keycloak アカウントのパスワードとロールは管理画面から変更できません（ロールは IdP 側の割り当てが正です）。

  ![管理 → ユーザー管理 — 認証列（ローカル / Keycloak）](images/manual/15-user-management.png)

- **APIトークン**: Keycloak アカウントが発行する PAT は最長 7 日で必ず失効します（IdP 側でのアカウント失効・降格は次回ログインまでこのサーバーに伝わらないため）。ローカルアカウントの PAT は従来どおりです。
- **ログアウト**: Keycloak でログインしたセッションのログアウトは Keycloak の SSO セッションも終了します（RP-Initiated Logout）。
