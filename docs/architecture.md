...
- **PDF レンダリング**: `PdfRenderer`（PDFBox）が要素レンダラレジストリ（`ElementPdfRendererRegistry`）とセクションレンダラレジストリ（`SectionPdfRendererRegistry`）に委譲。`FontProvider` が CJK フォント（Noto Sans JP / Noto Serif JP、serif 判定は `isSerifFamily` — 汎用 `serif`/Mincho 系を明朝に解決）を埋め込みます。フロントの帳票キャンバスも同一フォント由来の自己ホスト Web フォント（`public/fonts/*.woff2`、#317）で描画するため、プレビューと PDF の書体・グリフ幅が一致します。テキスト描画は em ベースの `letterSpacing` にも対応（#319）。
...
- **新しい要素タイプ**: 型定義 → `ELEMENT_TYPES` 追加 → Renderer/PropertiesPanel → `ElementDef`（`index.tsx`）→ `ELEMENT_REGISTRY` 登録 → ファクトリ → パレット登録 → PDF 側の要素レンダラ。手順は [設計 › 要素タイプの追加](./design.md#4-要素タイプの追加手順)。
