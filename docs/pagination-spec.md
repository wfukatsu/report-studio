...
> 補足（#276）: `SectionRenderHelper` は facade で、内部実装は `PagePlanBuilder` /
> `BandFlowPlanner` / `SectionGeometry` / `FormDataResolver` 等の package-private
> collaborators に分割されている。本書の `SectionRenderHelper.xxx` 参照は facade の
> 公開 API として引き続き有効（定数 `MAX_BAND_PAGES` のみ `BandFlowPlanner` に移動）。
...
- 継続ページ数は他のページ分割セクションと同様に文書ページ数の max に寄与
  （上限 200: `BandFlowPlanner.MAX_BAND_PAGES`）
